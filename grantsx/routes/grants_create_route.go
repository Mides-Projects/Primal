...
func GrantsCreateRoute(c echo.Context) error {
	...
	gaAddedBy := service.Grants().LookupAtCache(g.AddedBy())
	if gaAddedBy == nil {
		return common.HTTPError(http.StatusNotFound, "Grants for source adder not found")
	}

	if gaAddedBy.Account().Id() != g.AddedBy() {
		return common.HTTPError(http.StatusConflict, "Source adder ID mismatch")
	}

	hgAdder := service.Grants().HighestGroupBy(gaAddedBy)
	if hgAdder == nil {
